...
void *write_to_cloud(void *arg)
{
    ...
    while (1)
    {
        if (cloud_data != NULL)
        {
            if (cloud_data->gps_data.hdop >= GPS_ERROR_RANGE_BEGIN && cloud_data->gps_data.hdop <= GPS_ERROR_RANGE_END && cloud_data->client_controller_data.acc_x <= CLIENT_CONTROLLER_ERROR_RANGE_BEGIN)
            {
